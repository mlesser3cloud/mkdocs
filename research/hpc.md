Here's the updated markdown file with an exhaustive migration process section:
...
- Navigate to Azure Portal and create a dedicated resource group for HPC resources [web:3]
- Select appropriate Azure region based on latency requirements and service availability [web:25]
...
- Create a Virtual Network (VNet) with appropriate CIDR block for expected growth [web:3]
- Design subnet topology including separate subnets for management, compute, and storage [web:25]
...
- Configure Azure Active Directory integration for centralized authentication [web:24]
- Implement Role-Based Access Control (RBAC) with principle of least privilege [web:24]
...
- Review current Azure subscription quota limits for target VM families [web:1][web:10]
...
Choose appropriate storage solutions based on workload requirements [web:24]:

**For High-Performance Parallel Workloads:**
- Deploy Azure Managed Lustre for applications requiring maximum throughput and low latency [web:24]
...
- Provision Azure NetApp Files for enterprise-grade performance [web:24]
- Configure service levels (Standard, Premium, Ultra) based on IOPS requirements
- Set up snapshots and replication policies for data protection [web:24]
- Create volumes with appropriate protocols (NFSv3, NFSv4.1, SMB)

**For General-Purpose Storage:**
- Create Azure Storage Accounts with NFS v3 support [web:24]
...
1. Create NetApp Account in target resource group [web:24]
...
4. Obtain mount information from volume properties [web:24]

#### 2.3 Client Configuration
On each HPC compute node:

1. Install required client packages [web:24]:
   ```bash
   # For NFS
...
2. Configure mount points in `/etc/fstab` [web:24]:
   ```bash
   # Example NFS mount
...
Use Azure Data Box for offline transfer [web:20]:

1. Navigate to Azure Portal and order Azure Data Box [web:20]
...
5. Connect Data Box and copy data using provided tools [web:20]
6. Ship Data Box back to Azure datacenter
7. Verify data upload completion through Azure Portal [web:20]
8. Validate data integrity using checksums

**For Medium Datasets (1-10 TB):**
Use AzCopy or Azure File Sync [web:24]:
...
Deploy Azure File Sync or rsync [web:24]:
...
1. Access CycleCloud web interface at `https://<cyclecloud-ip>` [web:25]
2. Accept self-signed certificate warning (or configure proper SSL certificate) [web:25]
3. Create CycleCloud administrator account with strong password [web:25]
4. Configure Azure credentials:
   - Subscription ID
   - Tenant ID
   - Application ID (Service Principal)
   - Application Secret [web:23]
5. Validate credentials and verify Azure connectivity [web:23]
6. Set default region and resource group for cluster deployments [web:23]

#### 4.3 CycleCloud CLI Setup
On the CycleCloud server:

1. Initialize CycleCloud CLI [web:25]:
   ```bash
   cyclecloud initialize
   ```

2. Provide configuration parameters [web:25]:
   - CycleServer URL: `https://localhost`
   - Username: CycleCloud admin account
   - Password: Admin password
   - Accept untrusted certificate: Yes

3. Verify CLI configuration saved to `~/.cycle/config.ini` [web:25]
...
- **Slurm**: Most common open-source scheduler, supports cloud bursting [web:17]
...
- **Custom**: Import custom cluster templates [web:25]

#### 5.2 Cluster Configuration Parameters

**Basic Settings:**
- Cluster name and description [web:25]
- Scheduler type and version
- Region and availability zones [web:25]
- Virtual network and subnet selection [web:23][web:25]
...
- VM size families (HBv3, HBv2, HC44rs for MPI workloads) [web:22]
...
- NFS mount points from head node [web:23]
- External storage mounts (Lustre, NetApp Files) [web:22]
- Shared filesystem size (100 GB default, increase as needed) [web:23]
- Blob storage integration for backup (HSM functionality) [web:22]

**Advanced Settings:**
- Cloud-init scripts for custom software installation [web:23]
...
1. From CycleCloud web console, click **Clusters** > **+** (Add) [web:25]
2. Select scheduler template (e.g., Slurm, PBS Pro)
3. Configure cluster parameters using wizard:
   - **About** tab: Name and description
   - **Required Settings**: Azure configuration, networking [web:23]
   - **Compute**: VM sizes and autoscaling [web:23]
   - **Storage**: NFS and external mounts [web:23]
   - **Advanced**: Custom scripts and software
4. Review configuration summary
5. Click **Save** to create cluster definition
6. Click **Start** to begin deployment [web:23]
7. Monitor deployment progress:
   - Scheduler node creation [web:23]
   - Resource acquisition and IP assignment [web:23]
   - Software configuration [web:23]
   - Node template creation for compute nodes [web:23]
...
1. Install compilers (GCC, Intel, PGI) [web:22]
...
3. Verify autoscaling triggers compute node deployment [web:22]
...
3. Deploy Prometheus and Grafana for detailed metrics [web:13]
...
1. Analyze job patterns to right-size VM selections [web:1]
...
6. Monitor costs using Azure Cost Management [web:24]
...
5. Implement data tiering strategies for infrequently accessed data [web:24]
...
5. Implement encryption at rest for all storage [web:24]
6. Enable encryption in transit using TLS/SSL [web:24]
...
- **Leverage automation** through Azure Resource Manager templates and Infrastructure-as-Code approaches [web:1]
...
- **Plan for hybrid scenarios** if complete migration isn't feasible, enabling cloud bursting from on-premises to Azure [web:17]
- **Enable comprehensive monitoring** using Azure Monitor integration and custom dashboards for cluster health
- **Continuously reassess infrastructure** requirements to improve performance and reduce costs [web:1]
...
- Microsoft Azure Cloud Adoption Framework for HPC
- Azure CycleCloud Documentation
- Azure HPC Architecture Center
...
3. Paste the content
4. Save with the filename `azure-hpc-migration-strategy-complete.md`

This comprehensive guide now includes exhaustive step-by-step migration processes covering infrastructure setup, storage deployment, data migration, CycleCloud configuration, cluster deployment, workload migration, monitoring, and operational handoff.[1][2][3][4][5]

Sources
[1] Migrate Azure high-performance computing (HPC) https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/scenarios/azure-hpc/migrate
[2] Data migration https://learn.microsoft.com/en-us/azure/high-performance-computing/lift-and-shift-step-3-data-migration
[3] Storage migration options https://learn.microsoft.com/en-us/azure/high-performance-computing/lift-and-shift-step-3-storage
[4] HPC Lift and Shift Cloud Migration: Architecture and Best ... https://techcommunity.microsoft.com/blog/azurehighperformancecomputingblog/hpc-lift-and-shift-cloud-migration-architecture-and-best-practices/4260180
[5] How to create HPC Cluster based on Azure CycleCloud https://public-wiki.iucc.ac.il/index.php/How_to_create_HPC_Cluster_based_on_Azure_CycleCloud
[6] End-to-end lift and shift architecture overview https://learn.microsoft.com/en-us/azure/high-performance-computing/lift-and-shift-overview
[7] cloud-adoption-framework/docs/scenarios/azure-hpc ... https://github.com/MicrosoftDocs/cloud-adoption-framework/blob/main/docs/scenarios/azure-hpc/migrate.md
[8] Migrate & Optimize Energy/HPC Workloads to the Microsoft ... https://www.youtube.com/watch?v=tZAuz7_z4mc
[9] Deploying a CycleCloud cluster on Microsoft Azure https://www.youtube.com/watch?v=qIm6PAFVsmU
[10] Storage for Azure HPC Workloads - Cloud Adoption ... https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/scenarios/azure-hpc/storage
